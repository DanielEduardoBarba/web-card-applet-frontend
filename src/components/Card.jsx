...
export default function Card({c}) {
    ...
    useEffect(()=>{
        ...
        const objKeys = Object.keys(c)
        const _details = objKeys.map(key=>{
                        let detailObj={
                            name:key,
                            info:String(c[key])
                        }
        ...
        // const filteredDetails = _details.filter(obj=>{
        //     return c[key]
        // })
         setDetails(_details)           
                    ...
                    <div class="px-6 py-8 bg-white dark:bg-gray-800 lg:flex-shrink-1 lg:p-12">
                        ...
                        <div class="mt-8">
                            ...
                            <ul class="mt-8 lg:grid lg:grid-cols-2 lg:col-gap-8 lg:row-gap-5">
                               {
                                c?.links
                                    ?c?.links.map((l,i)=> <li class="flex items-start lg:col-span-1"> 
                                    <p class="ml-3 text-lg  text-gray-700 "> {l.name}</p>
                                    {l.link
                        ...
                        <div class="mt-8">
                            ...
                            {
                            details
                                ?details.map(dObj=>(<li class="flex items-start lg:col-span-1"> 
                                <p class="ml-3 text-lg  text-gray-700 "> {dObj.name}</p>
                                <p class="ml-3 text-lg  text-gray-700 "> {dObj.info}</p>
                               <svg class="w-6 h-6 mr-2" xmlns="http://www.w3.org/2000/svg" width="6" height="6" stroke="currentColor" fill="#10b981" viewBox="0 0 1792 1792">
                                    <path d="M1412 734q0-28-18-46l-91-90q-19-19-45-19t-45 19l-408 407-226-226q-19-19-45-19t-45 19l-91 90q-18 18-18 46 0 27 18 45l362 362q19 19 45 19 27 0 46-19l543-543q18-18 18-45zm252 162q0 209-103 385.5t-279.5 279.5-385.5 103-385.5-103-279.5-279.5-103-385.5 103-385.5 279.5-279.5 385.5-103 385.5 103 279.5 279.5 103 385.5z">
                                    </path>
                                </svg>  </li>))
